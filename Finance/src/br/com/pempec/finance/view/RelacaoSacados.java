...
import br.com.pempec.finance.models.reports.ReportSacado;
 ...
 * @author PEMPEC
 */
public class RelacaoSacados extends FinanceInternalFrame implements IRepopulador {

    SacadoBO sacadoBO = new SacadoBO();

    private String NameObject() {
        return (String) "Nova Data";
    }

    public RelacaoSacados() throws Exception {
    ...
    private void initComponents() {
        ...
        setTitle("PEMPEC ENTERPRISE - Finance - Relação Sacados");
        ...
        label1.setText("Listagem de Sacados");
    ...
    private void botaoGerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoGerarActionPerformed
        ...
        try {
            ...
            new MovimentoDiarioBO().inserir(this.registroMovimento("Lista Sacados", null, Controller.getUsuarioLogado().getNome() + " solicitou listagem de Sacados", null, "Impresso"));

            Map<String, Object> parametros = new HashMap<String, Object>();

            parametros.put(RelatorioConstantes.PARAMETRO_ENDERECO, organizacaoModel.getEndereco());

            List<SacadoModel> coll = sacadoBO.obterTodos(organizacaoModel);

            if (coll != null && coll.size() > 0) {

                Collection<ReportSacado> collection = new ArrayList<ReportSacado>();

                for (SacadoModel sacadoModel : coll) {

                    ReportSacado bean = new ReportSacado();

                    bean.setCnpj(organizacaoModel.getCnpj());
                    bean.setRazaoSocial(organizacaoModel.getRazaoSocial());


                    bean.setNome(sacadoModel.getNome());

                    bean.setCpfCnpj(sacadoModel.getCpfCnpj());
                ...
                switch (comboFormato.getSelectedIndex()) {
                    ...
                    case 0:
                        new RelatorioUtil().visualizar(RelatorioConstantes.RELATORIO_SACADOS, parametros, collection);
                        break;
                    case 1:
                        new RelatorioUtil().exportarPDF(RelatorioConstantes.RELATORIO_SACADOS, parametros, collection);
                        break;
                    case 2:
                        new RelatorioUtil().exportarXLS(RelatorioConstantes.RELATORIO_SACADOS, parametros, collection);
                        break;
                    case 3:

                        File anexo = new RelatorioUtil().exportarPDFtoFile(RelatorioConstantes.RELATORIO_SACADOS, parametros, collection);
