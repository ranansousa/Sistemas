...
import br.com.pempec.finance.models.reports.ReportFichaSacado;
 ...
 * @author PEMPEC
 */
public class FichaCadastralSacado extends FinanceInternalFrame implements IRepopulador {

    private long tela = Tela.TELA_PARAMETROS_SACADOS.getTela();
    private SacadoModel sacadoModel = new SacadoModel();

    public FichaCadastralSacado(SacadoModel sacado) throws SystemException {

        this.sacadoModel = sacado;
        initComponents();
    ...
    private void botaoGerarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoGerarActionPerformed
        ...
        if (validaCampos()) {
            ...
            try {
                ...
                Collection<ReportFichaSacado> collection = new ArrayList<ReportFichaSacado>();

                ReportFichaSacado bean = new ReportFichaSacado();

                if (sacadoModel != null && sacadoModel.getNome() != null) {

                    SacadoModel tab = new SacadoBO().consultarPorTemplate(sacadoModel);


                    bean.setAcumuladoReceber(0d);
                    bean.setAcumuladoRecebido(0d);
                    ...
                    bean.setTipoSacado(tab.getTipoSacado().getDescricao());

                    List<TituloReceberModel> collTitulos = new ArrayList<TituloReceberModel>();
                    Double valorRecebido = 0d;
                    Double valorReceber = 0d;

                    FilterReportTituloReceber filtro = new FilterReportTituloReceber();
                    ...
                    if (tab != null && tab.getPk().getId() != null) {
                        filtro.setSacado(tab.getPk().getId());
                        parametros.put(RelatorioConstantes.PARAMETRO_ID_SACADO, tab.getPk().getId());

                    }


                    collTitulos.addAll(new TituloReceberBO().obterRelatorio(filtro));
                    Long auxQtd = 0L;

                    for (TituloReceberModel tituloReceberModel : collTitulos) {
                        if (tituloReceberModel.getDataPagamento() != null) {
                            valorRecebido += tituloReceberModel.getValorNominal();

                        } else {

                            valorReceber += tituloReceberModel.getValorNominal();
                            if (tituloReceberModel.getDataVencimento().before(Controller.getDataServidorBD())) {
                                auxQtd++;
                            }
                    ...
                    bean.setAcumuladoReceber(valorReceber);

                    bean.setAcumuladoRecebido(valorRecebido);

                    bean.setTotal(valorRecebido, valorReceber);
                ...
                switch (comboFormato.getSelectedIndex()) {
                    ...
                    case 0:
                        new RelatorioUtil().visualizar(RelatorioConstantes.FICHA_CADASTRAL_SACADO, parametros, collection);
                        break;
                    case 1:
                        new RelatorioUtil().exportarPDF(RelatorioConstantes.FICHA_CADASTRAL_SACADO, parametros, collection);
                        break;
                    case 2:
                        new RelatorioUtil().exportarXLS(RelatorioConstantes.FICHA_CADASTRAL_SACADO, parametros, collection);
                        break;
                    case 3:

                        //Fazer tela de selecionar os destinatários. Pode ser múltiplos
                        File anexo = new RelatorioUtil().exportarPDFtoFile(RelatorioConstantes.FICHA_CADASTRAL_SACADO, parametros, collection);
